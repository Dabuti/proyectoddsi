...
try{
	...
	if($_GET["action"] == "list"){
        ...
        if (isset($_REQUEST["jtSorting"])){
            $query = "SELECT
                        idreceta, nombre, personas, tiempo, descripcion
                      FROM
                        receta
                      ORDER BY
                        $jsort";
	...
	else if($_GET["action"] == "create")
	{
        ...
        $tie = $_REQUEST['TIEMPO'];
 		$query = "INSERT INTO
             receta (
               nombre, personas, tiempo, descripcion
                   )
             VALUES (
               '$nombre', '$per', '$tie', '$des'
                    )
             ...
             INTO
               :receta_id";
        $statement = oci_parse($conn, $query);
        oci_bind_by_name($statement, ':receta_id', $receta_id, -1, SQLT_INT);
	...
	else if($_GET["action"] == "update")
	{
		...
		$query ="UPDATE
                   receta
                 SET
                   nombre = '$nombre', personas = '$per', descripcion = '$des', tiempo = '$tie'
                 WHERE
                   idreceta = '$id_receta'";
        $statement = oci_parse($conn, $query);
        oci_execute($statement);
