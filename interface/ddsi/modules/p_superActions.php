...
try{
	...
	if($_GET["action"] == "list"){
        ...
        if (isset($_REQUEST["jtSorting"])){
            $query = "SELECT
                        p.nombre nombrep, p.descripcion, p.calorias,
                        c.cantidad, c.precio,
                        s.nombre nombres,
                        to_char(com.fecha, 'yyyy-mm-dd') fecha
                      FROM
                        producto p, super s, compra com, compraproductosuper c
                      WHERE
                        c.idproducto = p.idproducto
                      and
                        c.idcompra = com.idcompra
                      and
                        c.idsuper = s.idsuper
                      and
                        com.idusuario = '$iduser'
                      ORDER BY
                        $jsort";
        }else{
            $query = "SELECT
